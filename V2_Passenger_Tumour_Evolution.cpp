...
g++ -c -I/$HOME/include ranlib_prb.cpp
	...
	/usr/local/bin/g++ -Wall -I/usr/local/include -c V2_Passenger_Tumour_Evolution.cpp --std=c++11
	/usr/local/bin/g++ -L/usr/local/lib V2_Passenger_Tumour_Evolution.o -fopenmp -lgsl -lgslcblas -lm
	rm V2_Passenger_Tumour_Evolution.o
	mv a.out V2_Passenger_Tumour_Evolution
	./V2_Passenger_Tumour_Evolution
...
const unsigned long long Pop_Size =   6000000000; //1000000000;
const unsigned long long Pop_Size_p = 6000000000; //1000000000;
const unsigned int stop_simulation_when_plateaus = 10;
const double driver_quantile = 1.75;//4.264891; // 5%
const double killer_quantile = -1.75; // 5%
const double beneficial_quantile = 0.2533471;// 10%
const double deleterious_quantile = -0.2533471;//10%
...
/****************************************************************************/
    ...
    /***********/
    {
        ...
        ************************************************************/
		...
		unsigned long long int Number_of_Passenger_Mutations; 
    ...
    ******************************************************/
	...
	double Random::Update_Proliferation_Rate(double Parent_Proliferation_Rate)
	{
		...
		double  U = Parent_Proliferation_Rate + 0.05;
		if(U > 1.0)
		{
			U = 1.0;
		}
    ...
    ****************************************************************************************************************/
	tuple<unsigned int,unsigned int, unsigned int, bool, bool> Basic_Clonal_Expansion(unique_ptr<Clonal_Expansion> const & CE, int Generation_ID, unsigned int hours)
	{
		...
		//cout << "DY: " << n[0] << " NB: " << n[1] << " NT: "<< n[2] << endl; 
			...
			long long int o = (long long int) ( CE -> Tumour -> at(Generation_ID) -> Clone_Size + n[1]) - (long long int) (n[0] + m);
	...
	void carcinogenesis_from_Driver(unique_ptr<Clonal_Expansion> const  & CE,  int Generation_ID , double driver_effect, unsigned int years, unsigned int hours)
	{
		...
		{	
			...
			unsigned long long int NP = CE -> Tumour -> at(Generation_ID) -> Number_of_Passenger_Mutations; 
			...
			CE -> Tumour -> back() -> Number_of_Passenger_Mutations = NP; 
		...
		if(Number_of_Mutants > 0)
		{
			...
			for(unsigned int j = 0; j < mutations; j++)
			{
				for(unsigned int i = 0; i < Number_of_Mutants; i++)
				{
					...
					else if( (muatational_effect > killer_quantile) && (muatational_effect <= deleterious_quantile) ) // Deleterious
					{
						CE -> Tumour -> at(Generation_ID) -> Number_of_Passenger_Mutations++;
						CE -> Tumour -> at(Generation_ID) -> P_Expansion[1] -= (double) (CE -> Tumour -> at(Generation_ID) -> Number_of_Passenger_Mutations)/(CE -> Tumour -> at(Generation_ID) -> Clone_Size);
						if(CE -> Tumour -> at(Generation_ID) -> P_Expansion[1]  < 0)
							CE -> Tumour -> at(Generation_ID) -> P_Expansion[1]  = 0;
					}
					else if( (muatational_effect < driver_quantile) && (muatational_effect >= beneficial_quantile)) // Beneficial
					{
						CE -> Tumour -> at(Generation_ID) -> Number_of_Passenger_Mutations++;
						CE -> Tumour -> at(Generation_ID) -> P_Expansion[1] -= (double) (CE -> Tumour -> at(Generation_ID) -> Number_of_Passenger_Mutations)/(CE -> Tumour -> at(Generation_ID) -> Clone_Size);
						if(CE -> Tumour -> at(Generation_ID) -> P_Expansion[1]  < 0)
					...
					else
					{
						...
						//	CE -> Tumour -> at(Generation_ID) -> P_Expansion[1] -= 0.0001;
						//Nothing
					}
				}//for
			}//poiss for
	...
	void compute_Mutations(unique_ptr<Clonal_Expansion> const & CE, int Generation_ID ,tuple<unsigned int, unsigned int, unsigned int, bool, bool> Dying_and_Newborn, unsigned int years, unsigned int hours )
	{
		...
		unsigned long long int New_poulation_Size = (
											(		
												CE -> Tumour -> at(Generation_ID) -> Clone_Size 
											 	-
    ...
    ***********************************************************/
	void compute_Tumour_Evolution()
	{
		...
		string path = "./V2_Passenger_Tumour_Evolution/";

  		unsigned int elapsed_hours = 0; 
  		ofstream Tumour_Evolution;
  		ofstream Pop_Stats;
  		Tumour_Evolution.open ("V2_Passenger_Tumour_Evolution.txt");
		...
		while( (times_to_wait < stop_simulation_when_plateaus) && (CE -> Population_Size > 0) )
		{
			seconds += dt;
			if(seconds == 3600)
			{
		...
		Pop_Stats.open ("V2_Passenger_Tumour_Evolution_stats.txt");
		Pop_Stats << "id\t" << "Clone_Size\t" << "Proliferation_Rate\t" << "Mutation_Rate\t" << "Extinct\t" << "G_ID\t"<< "Passengers"<<"\n";
  		...
  		for( ith_clone = 0; ith_clone < CE-> Tumour -> size() ; ith_clone ++)
  		{
  			Pop_Stats << ith_clone << "\t" 
  					  ...
  					  << CE -> Tumour -> at(ith_clone) -> Number_of_Passenger_Mutations << "\t"
